...
def h5_to_vtp(zvals, vtp_file="out", log=False, zmax=-1, interp=-1, chopxmin=0, chopxmax=0, chopymin=0, chopymax=0, bumpup=0):
    #set this to True to generate points
    show_points = False
    #set this to True to generate polygons
    show_polys = True

    xdim, ydim = zvals.shape
    assert xdim == ydim, "Array must be square of the vtk script fails"

    xcoordinates = [i for i in range(xdim)]
    ycoordinates = [i for i in range(xdim)]

    [xcoordinates, ycoordinates] = np.meshgrid(xcoordinates[chopxmin:-1-chopxmax], ycoordinates[chopymin:-1-chopymax])
    vals = zvals[chopxmin:-1-chopxmax,chopymin:-1-chopymax]

    x_array = xcoordinates[:].ravel()
    y_array = ycoordinates[:].ravel()
    z_array = vals[:].ravel()+bumpup
    z_array[np.logical_not(np.isfinite(z_array))] = zmax	
